use spdmlib2::requester::{self, VersionState as ReqVersionState};
use spdmlib2::responder::{self, VersionState as RspVersionState};
use spdmlib2::{msgs, Transcript};
...
fn successful_version_negotiation() {
    ...
    let req_size = req_state
        .write_get_version(&mut req_buf, &mut req_transcript)
        .unwrap();
    ...
    assert_eq!(
        responder::VersionTransition::Capabilities(responder::CapabilitiesState::new()),
        transition
    );
    ...
    assert_eq!(
        requester::VersionTransition::Capabilities(requester::CapabilitiesState::new(version)),
        transition
    );
